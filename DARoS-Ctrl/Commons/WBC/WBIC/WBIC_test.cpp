#include "WBIC_test.hpp"
#include <Utilities/Timer.h>
#include <eigen3/Eigen/LU>
#include <eigen3/Eigen/SVD>

template <typename T>
WBIC_test<T>::WBIC_test(size_t num_qdot, const std::vector<ContactSpec<T>*>* contact_list,
...
template <typename T>
void WBIC_test<T>::MakeTorque(DVec<T>& cmd, void* extra_input) {
  if (!this->b_updatesetting_) {
    printf("[Wanning] WBIC_test setting is not done\n");
  }
  ...
  if(_rf_computation_idx > (*_task_list).size()){ 
    _rf_computation_idx = (*_task_list).size(); 
  }

  for (size_t i(0); i < _rf_computation_idx; ++i) {
    task = (*_task_list)[i];
  ...
  for (size_t i(_rf_computation_idx); i < (*_task_list).size(); ++i) {
    task = (*_task_list)[i];
...
template <typename T>
void WBIC_test<T>::_SetEqualityConstraint(const DVec<T>& qddot) {
  if (_dim_rf > 0) {
  ...
  // pretty_print(_dyn_CE, std::cout, "WBIC_test: CE");
  // pretty_print(_dyn_ce0, std::cout, "WBIC_test: ce0");
}

template <typename T>
void WBIC_test<T>::_SetInEqualityConstraint() {
  ...
  // pretty_print(_dyn_CI, std::cout, "WBIC_test: CI");
  // pretty_print(_dyn_ci0, std::cout, "WBIC_test: ci0");
}

template <typename T>
void WBIC_test<T>::_ContactBuilding() {
  DMat<T> Uf;
  DVec<T> Uf_ieq_vec;
  // Initial
  DMat<T> Jc;
  ...
  //pretty_print(_Fr_des, std::cout, "[WBIC_test] Fr des");
  // pretty_print(_Jc, std::cout, "[WBIC_test] Jc");
  // pretty_print(_JcDotQdot, std::cout, "[WBIC_test] JcDot Qdot");
  // pretty_print(_Uf, std::cout, "[WBIC_test] Uf");
}

template <typename T>
void WBIC_test<T>::_GetSolution(const DVec<T>& qddot, DVec<T>& cmd) {
  DVec<T> tot_tau;
  if (_dim_rf > 0) {
...
template <typename T>
void WBIC_test<T>::UpdateSetting(const DMat<T>& A, const DMat<T>& Ainv,
...
template <typename T>
void WBIC_test<T>::_SetOptimizationSize() {
  // Dimension
  _dim_rf = 0;
...
template class WBIC_test<double>;
template class WBIC_test<float>;
