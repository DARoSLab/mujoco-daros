 ...
 */
DARoS_Simulation::DARoS_Simulation(FloatingBaseModel<double>* model,
  ...
  _simParams(params)
{
  ...
  // init parameters
  printf("[DARoS_Simulation] Load parameters...\n");
  ...
  if (_simParams.sim_state_lcm) {
    printf("[DARoS_Simulation] Setup LCM...\n");
  ...
  printf("[DARoS_Simulation] Build DARoS_Simulation Environment...\n");

  // init rigid body dynamics
  printf("[DARoS_Simulation] Build rigid body model...\n");
  ...
  printf("[DARoS_Simulation] Ready!\n");
}

/*!
 * Take a single timestep of dt seconds
 */
void DARoS_Simulation::step(
  ...
  if (_currentSimTime >= _timeOfNextLowLevelControl) {
    for (size_t idx(0); idx<_numActJoint; ++idx){
      actuator_cmd = actuator_models[idx]->computeLowLevelCMD(
          _dyn_engine->getState().q[idx], _dyn_engine->getState().qd[idx]);
...
void DARoS_Simulation::setExternalForce(const SVec<double> & f, int link_id){
  _dyn_engine->applyExternalForce(f, link_id);
}

void DARoS_Simulation::highLevelControl() {
...
void DARoS_Simulation::buildLcmMessage() {
 ...
 */
void DARoS_Simulation::addCollisionPlane(double mu, double resti, double height) {
 ...
 * @param addToWindow : if true, also adds graphics for the plane
 */
void DARoS_Simulation::addCollisionBox(double mu, double resti, double depth,
    double width, double height,
    const Vec3<double>& pos,
    const Mat3<double>& ori, bool addToWindow,
    bool transparent) {
  _dyn_engine->addCollisionBox(mu, resti, depth, width, height, pos, ori);
}

void DARoS_Simulation::addCollisionMesh(double mu, double resti, double grid_size,
...
void DARoS_Simulation::loadTerrainFile(const std::string& terrainFileName,
    bool addGraphics) {
