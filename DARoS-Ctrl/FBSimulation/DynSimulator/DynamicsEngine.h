...
#ifndef PROJECT_DYNAMICS_ENGINE_H
#define PROJECT_DYNAMICS_ENGINE_H
...
class DynamicsEngine {
 public:
  ...
  void setAllExternalForces(const vectorAligned<SVec<T>>& forces) {
    //printf("%lu external force size\n", _model->_externalForces.size());
    assert(forces.size() == _model->_externalForces.size());

    for(size_t i(0); i<_model->_externalForces.size(); ++i){
        _model->_externalForces[i] = forces[i];
    }
  }

  void applyExternalForce(const SVec<T> & f, int linkid){
    _model->_externalForces.at(linkid) = f;
  }
...
#endif  // PROJECT_DYNAMICS_ENGINE_H
