...
template <typename T>
Quat<typename T::Scalar> rpyToQuat(const Eigen::MatrixBase<T>& rpy, bool zyx = true) {
  static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 3,
      "Must have 3x1 vec");
Mat3<typename T::Scalar> m;
  if(zyx){
     m = coordinateRotation(CoordinateAxis::X, rpy[0]) *
  ...
  Quat<typename T::Scalar> q = rotationMatrixToQuaternion(m);
  return q;
}
