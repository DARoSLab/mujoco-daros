...
class JPosCtrl:
  ...
  def update(self):
    ...
    amp = 0.2
    time_offset = 0.
    target_position[7] = -amp*(1 - np.cos(2*np.pi * freq* (self.d.time - time_offset)))
    target_position[8] = -amp*(1 - np.cos(2*np.pi * freq* (self.d.time - time_offset)))
 
    # knee
    target_position[9] = amp*(1 - np.cos(2*np.pi * freq* (self.d.time - time_offset)))
    ...
    target_position[12] = -amp*(1 - np.cos(2*np.pi * freq* (self.d.time - time_offset)))
...
if __name__ == '__main__':
  ...
  def main(argv) -> None:
    launch_from_path(dir_path + "/../Robots/Staccatoe/staccatoe.xml")
