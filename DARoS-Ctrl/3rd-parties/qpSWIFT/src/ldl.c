...
* The int array Ai and the real array Ax are of size nz.  This data structure
...
* real array Lx of the same size as Li).  It has a unit diagonal, which is
...
void LDL_symbolic
(
qp_int n,		/* A and L are n-by-n, where n >= 0 */
qp_int Ap[],	/* input of size n+1, not modified */
qp_int Ai[],	/* input of size nz=Ap[n], not modified */
qp_int Lp[],	/* output of size n+1, not defined on input */
qp_int Parent[],	/* output of size n, not defined on input */
qp_int Lnz[],	/* output of size n, not defined on input */
qp_int Flag[],	/* workspace of size n, not defn. on input or output */
qp_int P[],	/* optional input of size n */
qp_int Pinv[]	/* optional output of size n (used if P is not NULL) */
)
{
	qp_int i, k, p, kk, p2;
	if (P)
...
qp_int LDL_numeric	/* returns n if successful, k if D (k,k) is zero */
(
qp_int n,		/* A and L are n-by-n, where n >= 0 */
qp_int Ap[],	/* input of size n+1, not modified */
qp_int Ai[],	/* input of size nz=Ap[n], not modified */
qp_real Ax[],	/* input of size nz=Ap[n], not modified */
qp_int Lp[],	/* input of size n+1, not modified */
qp_int Parent[],	/* input of size n, not modified */
qp_int Lnz[],	/* output of size n, not defn. on input */
qp_int Li[],	/* output of size lnz=Lp[n], not defined on input */
qp_real Lx[],	/* output of size lnz=Lp[n], not defined on input */
qp_real D[],	/* output of size n, not defined on input */
qp_real Y[],	/* workspace of size n, not defn. on input or output */
qp_int Pattern[],/* workspace of size n, not defn. on input or output */
qp_int Flag[],	/* workspace of size n, not defn. on input or output */
qp_int P[],	/* optional input of size n */
qp_int Pinv[]	/* optional input of size n */
)
{
	qp_real yi, l_ki, Sign, delta, eps;
	delta = 0.0000001;
	eps = 0.00000000000001;
	qp_int i, k, p, kk, p2, len, top;
	for (k = 0; k < n; k++)
	{
		...
		Sign = D[k] <= 0 ? -1.0 : 1.0;
		D[k] = Sign * D[k] <= eps ? Sign * delta : D[k];
		/* if (D [k] == 0.0) return (k) ;  */   /* failure, D(k,k) is zero */ 
	}
	/* printf("First Time: %f\tSecond Time:%f\n", ntone, nttwo); */
	/* printf("First one:%f\tSecond one:%f\n", none, ntwo); */
...
/* Given a sparse matrix A (the arguments n, Ap, Ai, and Ax) and its symbolic
* analysis (Lp and Parent, and optionally P and Pinv), and the sparsity pattern
* of the transpose of the choleksy factor (Lti,Ltp), compute the numeric LDL'
* factorization of A or PAP' columnwise (Left-looking choelsky factorization).
* The outputs of this routine are arguments Li,
* Lx, and D.  It also requires three size-n workspaces (Y, Pattern, and Flag).
*/


qp_int LDL_cache_numeric	/* returns n if successful, k if D (k,k) is zero */
(
qp_int n,		/* A and L are n-by-n, where n >= 0 */
qp_int Ap[],	/* input of size n+1, not modified */
qp_int Ai[],	/* input of size nz=Ap[n], not modified */
qp_real Ax[],	/* input of size nz=Ap[n], not modified */
qp_int Ltp[],	/* input of size n+1, not modified */
qp_int Lti[],	/* input of size lnz=Lp[n], not modified*/
qp_int Li[], /* output of size lnz=Lp[n], not defined on input */
qp_int Lp[], /* input of size n+1, not modified */
qp_real Lx[],	/* output of size lnz=Lp[n], not defined on input */
qp_real D[],	/* output of size n, not defined on input */
...
qp_int P[],	/* optional input of size n */
qp_int Pinv[],	/* optional input of size n */
qp_int UPattern[] /*Update Pattern of subsequent iterations*/
)
{
	qp_real  l_ki;
	qp_int i, k, p, kk, jj, tt;
	qp_int *work;
	qp_int col_idx;
	qp_int Sign;
	qp_real delta = 0.0000001;
	qp_real eps = 0.00000000000001;
	work = (qp_int*)MALLOC(n*sizeof(qp_int));

	for (k = 0; k < n; k++) { work[k] = Lp[k]; } /* Workspace to keep track of current row index */

	for (k = 0; k < n; k++)
	{

		if (UPattern[k] == n){        /* Columns that change */

			kk = (P) ? (P[k]) : (k);  /* kth original, or permuted, column */
			for (p = Ap[kk]; p < Ap[kk + 1]; p++)
			{
				i = (Pinv) ? (Pinv[Ai[p]]) : (Ai[p]);	/* get A(i,k) */
				if (i >= k)
				{
			...
			for (jj = Ltp[k]; jj < Ltp[k + 1]; jj++){
				col_idx = Lti[jj];       /* Obtaining non-zero row pattern */
				tt =  work[col_idx]++;
				l_ki = Lx[tt] * D[col_idx];
				for (; tt < Lp[col_idx + 1]; tt++){
					Y[Li[tt]] -= Lx[tt] * l_ki;   /* Performing sparse matrix vector */
				}
			}


			Sign = Y[k] <= 0 ? -1 : 1;						/*Dynamic Regularization*/
			D[k] = Sign * Y[k] <= eps ? Sign * delta : Y[k];
			Y[k] = 0;
			l_ki = D[k];

			for (tt = Lp[k]; tt < Lp[k + 1]; tt++){      /*Computing kth column of Matrix*/
				Lx[tt] = Y[Li[tt]] / l_ki;
				Y[Li[tt]] = 0.0;
			}
		}
			...
			/*for (jj = Ltp[k]; jj < Ltp[k + 1]; jj++)
				++work[Lti[jj]];*/
			for (jj = Lp[k]; UPattern[Li[jj]] != n && jj < Lp[k + 1]; jj++, work[k]++);
		}
...
* analysis (Lp and Parent, and optionally P and Pinv), and the sparsity pattern
* of the transpose of the choleksy factor (Lti,Ltp), compute the numeric LDL'
* factorization of A or PAP' row-wise (Up-looking choelsky factorization).
* The outputs of this routine are arguments Li,
* Lx, and D.  It also requires three size-n workspaces (Y, Pattern, and Flag).
*/

qp_int LDL_row_cache_numeric	/* returns n if successful, k if D (k,k) is zero */
(
qp_int n,		/* A and L are n-by-n, where n >= 0 */
qp_int Ap[],	/* input of size n+1, not modified */
qp_int Ai[],	/* input of size nz=Ap[n], not modified */
...
qp_int P[],	/* optional input of size n */
qp_int Pinv[],	/* optional input of size n */
qp_int UPattern[] /*Update Pattern of subsquent iterations*/
)
{
	qp_real yi, l_ki, Sign, eps, delta;
	delta = 0.0000001;
	eps = 0.00000000000001;
	qp_int i, k, p, kk, jj;
	for (k = 0; k < n; k++)
	{
		...
		if (UPattern[k] == n) {
			...
			for (jj = Ltp[k]; jj < Ltp[k + 1]; jj++)
			{
				i = Lti[jj];	    /* Non - zero Pattern of kth row of L */
				yi = Y[i];	        /* get and clear Y(i) */
				Y[i] = 0.0;
				for (p = Lp[i]; Li[p] < k; p++)
				{
...
void LDL_lsolve
(
qp_int n,		/* L is n-by-n, where n >= 0 */
qp_real X[],	/* size n.  right-hand-side on input, soln. on output */
qp_int Lp[],	/* input of size n+1, not modified */
qp_int Li[],	/* input of size lnz=Lp[n], not modified */
qp_real Lx[]	/* input of size lnz=Lp[n], not modified */
)
{
	qp_int j, p, p2;
	for (j = 0; j < n; j++)
...
void LDL_dsolve
(
qp_int n,		/* D is n-by-n, where n >= 0 */
qp_real X[],	/* size n.  right-hand-side on input, soln. on output */
qp_real D[]	/* input of size n, not modified */
)
{
	qp_int j;
	for (j = 0; j < n; j++)
...
void LDL_perm
(
qp_int n,		/* size of X, B, and P */
qp_real X[],	/* output of size n. */
qp_real B[],	/* input of size n. */
qp_int P[]	/* input permutation array of size n. */
)
{
	qp_int j;
	for (j = 0; j < n; j++)
...
qp_int LDL_valid_perm	    /* returns 1 if valid, 0 otherwise */
(
qp_int n,
qp_int P[],	    /* input of size n, a permutation of 0:n-1 */
qp_int Flag[]	    /* workspace of size n */
)
{
	qp_int j, k;
	if (n < 0 || !Flag)
