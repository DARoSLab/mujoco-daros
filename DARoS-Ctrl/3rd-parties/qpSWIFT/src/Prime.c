...
QP* QP_SETUP_dense(qp_int n, qp_int m, qp_int p, qp_real *Ppr, qp_real *Apr, qp_real *Gpr, qp_real *c, qp_real *h, qp_real *b,qp_int *Permut) {
	...
	myQP->P = (smat*)MALLOC(sizeof(smat)); densetosparse(myQP->n, myQP->n, Ppr, myQP->P);
	myQP->G = (smat*)MALLOC(sizeof(smat)); densetosparse(myQP->m, myQP->n, Gpr, myQP->G);
	myQP->c = c;
	myQP->h = h;
	myQP->sigma_d = 0.0;
...
void QP_CLEANUP_dense(QP *myQP) {
	...
	/* Free myQP Matrices */
	FREE(myQP->P->pr); FREE(myQP->P->jc); FREE(myQP->P->ir); FREE(myQP->P);
	FREE(myQP->G->pr); FREE(myQP->G->jc); FREE(myQP->G->ir); FREE(myQP->G);
	if (myQP->A) {
		FREE(myQP->A->pr); FREE(myQP->A->jc); FREE(myQP->A->ir); FREE(myQP->A);
		FREE(myQP->At->ir);
		FREE(myQP->At->jc);
		FREE(myQP->At->pr);
		FREE(myQP->At);
