...
#include "../../Utils/Utilities/orientation_tools.h"
      ...
      while(mip_get_next_field(packet, &field_header,
        ...
        switch(field_header->descriptor) {
          case MIP_FILTER_DATA_ATT_QUATERNION:
          {
            ...
            // Up Xtreme config
            //g_R_imu << 0, -1, 0, -1, 0, 0, 0, 0, -1;
            //r_R_imup << 0, -1, 0, 0, 0, -1, 1, 0, 0;
            //R_imu = ori::quaternionToRotationMatrix(gLordImu->quat);

            // r_R_imup << 0, 0, 1, -1, 0, 0, 0, -1, 0;
            // r_R_imup << 1, 0, 0, 0, 1, 0, 0, 0, 1;

//             printf("Q imu\n");
//             std::cout<<gLordImu->quat<<std::endl;
// printf("R imu\n");
//             std::cout<<R_imu<<std::endl;
// printf("\n\n");

            Vec4<float> ql = ori::rotationMatrixToQuaternion(g_R_imu.transpose());
            Vec4<float> qr = ori::rotationMatrixToQuaternion(r_R_imup.transpose());
      ...
      while(mip_get_next_field(packet, &field_header,
        ...
        switch(field_header->descriptor) {
          case MIP_AHRS_DATA_ACCEL_SCALED:
            ...
            gLordImu->acc[2] = -9.81f * accel.scaled_accel[1];
          ...
          case MIP_AHRS_DATA_GYRO_SCALED:
            ...
            gLordImu->gyro[2] = -gyro.scaled_gyro[1];
