...
bool BodyPosTask<T>::_UpdateCommand(const void* pos_des, const DVec<T>& vel_des,
  ...
  // TODO: position command is in global frame?
...
bool BodyPosTask<T>::_UpdateTaskJDotQdot() {
  Quat<T> quat = _robot_sys->_state.bodyOrientation;
  Mat3<T> Rot = ori::quaternionToRotationMatrix(quat);
  Vec3<T> loc_omega=_robot_sys->_state.bodyVelocity.head(3);
  Vec3<T> loc_v=_robot_sys->_state.bodyVelocity.tail(3);
  TK::JtDotQdot_ = Rot.transpose()*(loc_omega.cross(loc_v));
  return true;
}
