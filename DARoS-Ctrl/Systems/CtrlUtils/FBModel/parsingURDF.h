#ifndef PARSING_URDF_H
#define PARSING_URDF_H
...
bool buildFloatingBaseModelFromURDF(FloatingBaseModel<T> & model, const std::string & urdf_file, bool verbose = false){
  ...
  Vec3<T> root_inertial_rpy;
  Vec3<T> root_inertial_position;
  Mat3<T> root_inertial_inertia;
  ...
  SpatialInertia<T> root_inertia(root_inertial_mass, root_inertial_position, root_inertial_inertia);
  model.addBase(root_inertia, urdf_model->getRoot()->name);
  const int baseID = 5;
  int bodyID = baseID;
  ...
  for (size_t j(0); j < joint_names.size(); ++j) {
   ...
   // create the joint
    ...
    if (urdf_parent->name != "base_link") {
      parent_id = model.getBodyID(urdf_parent->name.c_str());
    }
    ...
    // assemble the body
    Vec3<T> link_inertial_position;
    Vec3<T> link_inertial_rpy;
    Mat3<T> link_inertial_inertia = Mat3<T>::Zero();
    ...
    SpatialInertia<T> link_inertia(link_inertial_mass, link_inertial_position, link_inertial_inertia);
    ...
    SpatialInertia<T> rotor_inertia_dummy(0.1e-5, Vec3<T>::Zero(), smallRotorRotationalInertiaZ);
    int gearRatio(9);

    Mat3<T> I3 = Mat3<T>::Identity();
    Mat6<T> Xtree = createSXform(ori::rpyToRotMat(joint_rpy), joint_translation);
    Mat6<T> Xrotor = createSXform(I3, Vec3<T>::Zero());
    ...
    model.addBody(link_inertia, rotor_inertia_dummy, gearRatio, parent_id, 
