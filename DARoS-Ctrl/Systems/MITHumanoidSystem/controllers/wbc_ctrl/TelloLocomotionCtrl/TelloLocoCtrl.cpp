...
template<typename T>
TelloLocoCtrl<T>::TelloLocoCtrl(const FloatingBaseModel<T> * model, const ControlFSMData_Tello<T>* _controlFSMData):
  WBC_Ctrl<T>(model)
{
...
template<typename T>
void TelloLocoCtrl<T>::_ContactTaskUpdate(void* input, ControlFSMData_Tello<T> & data){
  _input_data = static_cast<TelloLocoCtrlData<T>* >(input);
...
template<typename T>
void TelloLocoCtrl<T>::_ParameterSetup(const TelloParameters* param){
...
template<typename T>
void TelloLocoCtrl<T>::_CleanUp(){
  this->_contact_list.clear();
  this->_task_list.clear();
}

template<typename T>
void TelloLocoCtrl<T>::_LCM_PublishData() {
...
template class TelloLocoCtrl<float>;
