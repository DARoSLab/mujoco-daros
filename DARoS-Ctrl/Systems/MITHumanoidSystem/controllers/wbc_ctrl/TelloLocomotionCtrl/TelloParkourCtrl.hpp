#ifndef Tello_Parkour_CONTROLLER
#define Tello_Parkour_CONTROLLER

#include <wbc_ctrl/WBC_Ctrl.hpp>

template<typename T>
class TelloParkourCtrlData{
  public:
    Vec3<T> pBody_des;
    Vec3<T> vBody_des;
    Vec3<T> aBody_des;
...
template<typename T>
class TelloParkourCtrl: public WBC_Ctrl<T>{
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    TelloParkourCtrl(const FloatingBaseModel<T> * model, const ControlFSMData_Tello<T>* _controlFSMData);
    virtual ~TelloParkourCtrl();
  ...
  protected:
    ...
    TelloParkourCtrlData<T>* _input_data;
