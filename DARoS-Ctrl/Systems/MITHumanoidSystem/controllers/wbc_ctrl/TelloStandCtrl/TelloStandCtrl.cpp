...
template<typename T>
TelloStandCtrl<T>::TelloStandCtrl(const FloatingBaseModel<T> * model):
  WBC_Ctrl<T>(model)
{
...
template<typename T>
TelloStandCtrl<T>::~TelloStandCtrl(){
...
template<typename T>
void TelloStandCtrl<T>::_ContactTaskUpdate(void* input, ControlFSMData_Tello<T> & data){
  _input_data = static_cast<TelloStandCtrlData<T>* >(input);
...
template<typename T>
void TelloStandCtrl<T>::_ParameterSetup(const TelloParameters* param){
  ...
  for(size_t jindx(0); jindx<tello::num_act_joint; ++jindx){
    ((JPosTask<T>*)_jpos_task)->_Kp[jindx] = 150.;
    ((JPosTask<T>*)_jpos_task)->_Kd[jindx] = 0.5;
  }
...
template<typename T>
void TelloStandCtrl<T>::_CleanUp(){
  this->_contact_list.clear();
  this->_task_list.clear();
}

template<typename T>
void TelloStandCtrl<T>::_LCM_PublishData() {
...
template class TelloStandCtrl<float>;
