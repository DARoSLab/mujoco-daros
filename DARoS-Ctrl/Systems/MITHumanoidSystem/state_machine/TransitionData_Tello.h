/*============================= FSM State =============================*/
#ifndef TRANSITIONDATA_Tello_H
#define TRANSITIONDATA_Tello_H

#include <cppTypes.h>

/**
 * Struct of relevant data that can be used during transition to pass
 * data between states.
 */
template <typename T>
struct TransitionData_Tello {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  TransitionData_Tello() { zero(); }

  // Zero out all of the data
  void zero() {
    // Flag to mark when transition is done
    done = false;

    // A flag to set a specific transition number
    transition_type = 0;

    // Timing parameters
    t0 = 0.0;         // time that transition started
    tCurrent = 0.0;   // current time since transition started
    tDuration = 0.0;  // overall transition duration

    // Robot state at the beginning of transition
    comState0.setZero();   // center of mass state
    qJoints0.setZero();    // joint positions
    pFoot0.setZero();      // foot positions

    // Current robot state
    comState.setZero(); // center of mass state
    qJoints.setZero();   // joint positions
    pFoot.setZero();     // foot positions
  }

  // Flag to mark when transition is done
  bool done = false;

  // A flag to set a specific transition number
  int transition_type;

  // Timing parameters
  T t0;         // time that transition started
  T tCurrent;   // current time since transition started
  T tDuration;  // overall transition duration

  // Robot state at the beginning of transition
  Vec12<T> comState0;  // center of mass state
  Vec25<T> qJoints0;   // joint positions
  Mat34<T> pFoot0;     // foot positions

  // Current robot state
  Vec12<T> comState;  // center of mass state
  Vec25<T> qJoints;   // joint positions
  Mat34<T> pFoot;     // foot positions
};

template struct TransitionData_Tello<double>;
template struct TransitionData_Tello<float>;

#endif  // CONTROLFSM_H
