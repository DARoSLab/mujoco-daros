...
#include <wbc_ctrl/TelloLocomotionCtrl/TelloParkourCtrl.hpp>
 ...
 */
template <typename T>
Tello_State_Parkour<T>::Tello_State_Parkour(
    ControlFSMData_Tello<T>* _controlFSMData)
    : Tello_State<T>(_controlFSMData, Tello_StateName::PARKOUR,"PARKOUR") {
  // TEST
  this->turnOffAllSafetyChecks();

  _casadiMPC = new Parkour_RL<T>(_controlFSMData->_userParameters->controller_dt,
  ...
  _wbc_ctrl = new TelloParkourCtrl<T>(this->_model, _controlFSMData);
  _wbc_data = new TelloParkourCtrlData<T>();
  ...
  printf("[Casadi PARKOUR State] Constructed\n");
}

template <typename T>
void Tello_State_Parkour<T>::onEnter() {
  this->UpdateModel();
  ...
  pretty_print(_ini_com_pos, std::cout, "[Tello PARKOUR] ini com pos");
  pretty_print(_mid_pos_cps, std::cout, "[Tello PARKOUR middle of cps");
  std::cout << "[Tello PARKOUR] On Enter" << std::endl;
}
 ...
 */
template <typename T>
void Tello_State_Parkour<T>::run() {
 ...
 */
template <typename T>
Tello_StateName Tello_State_Parkour<T>::checkTransition() {
  this->nextStateName = this->stateName;
  _iter++;
  ...
  switch ((int)this->_fsm_data->_userParameters->control_mode) {
    ...
    case Tello_StateName::PARKOUR:  // normal c (0)
      // Normal operation for state based transitions
      break;
 ...
 */
template <typename T>
TransitionData_Tello<T> Tello_State_Parkour<T>::transition() {
  // Finish Transition
 ...
 */
template <typename T>
void Tello_State_Parkour<T>::onExit() {
  _iter = 0;
}

template <typename T>
void Tello_State_Parkour<T>::KeepPostureStep() {
  ...
  _wbc_data->jpos_des[tello::shoulder_roll_R] = 0.0;
  _wbc_data->jpos_des[tello::shoulder_roll_L] = 0.0;
  _wbc_data->jpos_des[tello::shoulder_yaw_R] = 0.0;
  _wbc_data->jpos_des[tello::shoulder_yaw_L] = 0.0;
  _wbc_data->jpos_des[tello::shoulder_pitch_R] = 0;//(this->_state.q[tello::L_hipRy] + 1)*0.4;
  _wbc_data->jpos_des[tello::shoulder_pitch_L] = 0;//(this->_state.q[tello::R_hipRy] + 1)*0.4;
  _wbc_data->jpos_des[tello::elbow_R] = -0.5;
  _wbc_data->jpos_des[tello::elbow_L] = -0.5;
  // std::cout<<"joint pos : "<< this->_state.q.transpose() << std::endl;
...
template class Tello_State_Parkour<float>;
