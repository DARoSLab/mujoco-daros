...
void Reversal_LIPM_Planner::getSWitchStateAndNextFootLoc(double t_swing,
                                   const Vec3<float> & com_pos_ini,
                                   const Vec3<float> & com_vel_ini,
                                   const Vec3<float> & stance_foot_loc_ini,
                                   Vec3<float> & com_pos,
                                   Vec3<float> & com_vel,
  ...
  _computeSwitchingState(t_swing, com_pos_ini, com_vel_ini,
          stance_foot_loc_ini, switch_state);
  for(int i(0); i<2; ++i){
    double exp_weight = (exp(omega_ * t_prime_[i]) + exp(-omega_ * t_prime_[i]))
        /(exp(omega_ * t_prime_[i]) - exp(-omega_ * t_prime_[i]));
    com_pos[i] = switch_state[i][0];
    com_vel[i] = switch_state[i][1];
    target_loc[i] = (1-kappa_[i])*com_pos[i] + (com_vel[i]/omega_) * exp_weight;
  }
  com_pos[2] = 0;
  com_vel[2] = 0;
  target_loc[2] = 0;
