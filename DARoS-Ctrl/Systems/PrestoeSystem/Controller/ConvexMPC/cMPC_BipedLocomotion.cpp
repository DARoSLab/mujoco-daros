  ...
  // sehwan - shorter stances for running
{
  ...
  _planner->PlannerInitialization(THIS_COM"config/pat_locomotion_parameters.yaml");
...
void cMPC_BipedLocomotion::solveDenseMPC(int *mpcTable, ControlFSMData<float> &data) {
 ...
 Eigen::Matrix<float,3*NUM_CONTACTS,1> u_input;
  ...
  // Eigen::Matrix<float,13,3*NUM_CONTACTS> B_c = get_B(); // B matrix in state space
  //
