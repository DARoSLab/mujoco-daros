...
MinJerk_OneDimension::MinJerk_OneDimension(const Vec3<float> & init, const Vec3<float> & end,
...
void MinJerk_OneDimension::Initialization(){
	...
	C_mat = DMat<float>::Zero(6,6);
	C_mat_inv = DMat<float>::Zero(6,6);
	a_coeffs = DVec<float>::Zero(6);
	bound_cond = DVec<float>::Zero(6);

	init_cond = DVec<float>::Zero(3);
	end_cond = 	DVec<float>::Zero(3);
	to = 0.0;
	tf = 1.0;
}

void MinJerk_OneDimension::setParams(const Vec3<float> & init, const Vec3<float> & end,
 			  const double & time_start, const double & time_end){
	// Set the Parameters
	init_cond = init;
	end_cond = end;
...
void MinJerk_OneDimension::printParameters(){
	...
	pretty_print(init_cond, std::cout, "init_cond");
	pretty_print(end_cond, std::cout,  "end_cond");
	pretty_print(C_mat, std::cout, "Coeff_Matrix");
	pretty_print(C_mat_inv, std::cout, "C_mat_inv");

	pretty_print(a_coeffs, std::cout, "Coeff_Vector");
	pretty_print(bound_cond, std::cout, "Boundary Conditions");
}
