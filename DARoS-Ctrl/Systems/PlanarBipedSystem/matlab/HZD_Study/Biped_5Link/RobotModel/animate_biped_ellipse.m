function animate_biped_ellipse(tspan, z_trj, fig, opt_check)
  if nargin < 4
    opt_check = false;
  end
    figure(fig)
...
%     circle = [0.05*cos(theta); 0.05*sin(theta)];
    ...
    for k =1:skip_step:len        
        ...
        % Left leg
        draw_lines([keypoints(:,hip_idx) keypoints(:,4:5)], h_leg_left);
        
        % Right leg
        draw_lines(keypoints(:,hip_idx:3), h_leg_right);
