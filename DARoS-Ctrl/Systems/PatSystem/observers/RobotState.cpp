...
float RobotState::getRelativeHeight() { 
#if INEKF_USE_MUTEX
    unique_lock<mutex> mlock(mutex_);
#endif
    float rel_height = 0;
    for (int i = 0; i < dimX() - 5; i++)
        rel_height += X_(2,4) - X_(2,5+i);
    rel_height /= (float) (dimX() - 5);
    return rel_height; 
}
int RobotState::dimX() { 
#if INEKF_USE_MUTEX
...
void RobotState::copyDiagXLeftInvariant(int n, Eigen::MatrixXd& BigX) {
    int dimX = this->dimX();
    for(int i=0; i<n; ++i) {
        int startIndex = BigX.rows();
        BigX.conservativeResize(startIndex + dimX, startIndex + dimX);
        ...
        unique_lock<mutex> mlock(mutex_);
#endif
        BigX.block(startIndex,startIndex,dimX,dimX) = X_.inverse();
    }
    return;
}
