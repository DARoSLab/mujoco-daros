...
// ------------ Observation -------------
// Default constructor
Observation::Observation(Eigen::VectorXd& Y_, Eigen::VectorXd& b_, Eigen::MatrixXd& H_, Eigen::MatrixXd& N_, Eigen::MatrixXd& PI_) :
    Y(Y_), b(b_), H(H_), N(N_), PI(PI_) {}
...
void InEKF::CorrectRI(const Observation& obs) {
    // Compute Kalman Gain
    Eigen::MatrixXd P = state_.getP();
...
void InEKF::CorrectLI(const Observation& obs) {
    int dimX = state_.dimX();
    int dimP = state_.dimP();
    int dimTheta = state_.dimTheta();

    // Convert right invariant covariance to left invariant
    Eigen::MatrixXd X = state_.getX();
    Eigen::MatrixXd Pr = state_.getP();

    Eigen::MatrixXd I = Eigen::MatrixXd::Identity(dimP,dimP);
    Eigen::MatrixXd AdjInv = I;
    AdjInv.block(0,0,dimP-dimTheta,dimP-dimTheta) = Adjoint_SEK3(X.inverse()); // Approx 200 microseconds
    Eigen::MatrixXd Adj = I;
    Adj.block(0,0,dimP-dimTheta,dimP-dimTheta) = Adjoint_SEK3(X); // Approx 200 microseconds

    Eigen::MatrixXd Pl = AdjInv * Pr * AdjInv.transpose();

    // Compute Kalman Gain
    Eigen::MatrixXd PHT = Pl * obs.H.transpose();
    Eigen::MatrixXd S = obs.H * PHT + obs.N;
    Eigen::MatrixXd K = PHT * S.inverse();

    // Copy X along the diagonals if more than one measurement
    Eigen::MatrixXd BigX;
    state_.copyDiagXLeftInvariant(obs.Y.rows()/state_.dimX(), BigX);
    ...
    // Update Covariance
    Eigen::MatrixXd IKH = Eigen::MatrixXd::Identity(state_.dimP(),state_.dimP()) - K*obs.H;
    Eigen::MatrixXd Pl_new = IKH * Pl * IKH.transpose() + K*obs.N*K.transpose(); // Joseph update form

    // Convert left invariant covariance back to right invariant
    Eigen::MatrixXd Pr_new = Adj * Pl_new * Adj.transpose();
   ...
   for (vectorKinematicsIterator it=measured_kinematics.begin(); it!=measured_kinematics.end(); ++it) {
    ...
    if (new_contacts.size() > 0) {
        ...
        for (vectorKinematicsIterator it=new_contacts.begin(); it!=new_contacts.end(); ++it) {
            ...
            // Initialize new kinematic covariance - TODO:speed up
            Eigen::MatrixXd F = Eigen::MatrixXd::Zero(state_.dimP()+3,state_.dimP()); 
            F.block(0,0,state_.dimP()-state_.dimTheta(),state_.dimP()-state_.dimTheta()) = Eigen::MatrixXd::Identity(state_.dimP()-state_.dimTheta(),state_.dimP()-state_.dimTheta()); // for old X
            F.block(state_.dimP()-state_.dimTheta(),6,3,3) = Eigen::Matrix3d::Identity(); // for new landmark
            F.block(state_.dimP()-state_.dimTheta()+3,state_.dimP()-state_.dimTheta(),state_.dimTheta(),state_.dimTheta()) = Eigen::MatrixXd::Identity(state_.dimTheta(),state_.dimTheta()); // for theta
...
// Correct state using kinematics measured between imu and contact point AND localization sensor
void InEKF::CorrectKinematicsAndLocalization (const vectorKinematics& measured_kinematics,
    Eigen::Vector3d p_glb, Eigen::Matrix3d cov) {
#if INEKF_USE_MUTEX
    lock_guard<mutex> mlock(estimated_contacts_mutex_);
#endif
    Eigen::VectorXd Yr, Yl;
    Eigen::VectorXd br, bl;
    Eigen::MatrixXd Hr, Hl;
    Eigen::MatrixXd Nr, Nl;
    Eigen::MatrixXd PI;
   ...
   for (vectorKinematicsIterator it=measured_kinematics.begin(); it!=measured_kinematics.end(); ++it) {
        ...
        // If contact is indicated and id is found in estimated_contacts_, then correct using kinematics
            ...
            // Fill out Y (right invariant)
            startIndex = Yr.rows();
            Yr.conservativeResize(startIndex+dimX, Eigen::NoChange);
            Yr.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            Yr.segment(startIndex,3) = it->pose.block<3,1>(0,3); // p_bc
            Yr(startIndex+4) = 1; 
            Yr(startIndex+it_estimated->second) = -1;

            // Fill out Y (left invariant)
            startIndex = Yl.rows();
            Yl.conservativeResize(startIndex+dimX, Eigen::NoChange);
            Yl.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            Yl.segment(startIndex,3) = p_glb; // global position
            Yl(startIndex+4) = 1; 
            Yl(startIndex+it_estimated->second) = 0;     

            // Fill out b (right invariant)
            startIndex = br.rows();
            br.conservativeResize(startIndex+dimX, Eigen::NoChange);
            br.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            br(startIndex+4) = 1;       
            br(startIndex+it_estimated->second) = -1;

            // Fill out b (left invariant)
            startIndex = bl.rows();
            bl.conservativeResize(startIndex+dimX, Eigen::NoChange);
            bl.segment(startIndex,dimX) = Eigen::VectorXd::Zero(dimX);
            br(startIndex+4) = 1;       
            br(startIndex+it_estimated->second) = 0;    

            // Fill out H (right invariant)
            startIndex = Hr.rows();
            Hr.conservativeResize(startIndex+3, dimP);
            Hr.block(startIndex,0,3,dimP) = Eigen::MatrixXd::Zero(3,dimP);
            Hr.block(startIndex,6,3,3) = -Eigen::Matrix3d::Identity(); // -I
            Hr.block(startIndex,3*it_estimated->second-6,3,3) = Eigen::Matrix3d::Identity(); // I

            // Fill out H (left invariant)
            startIndex = Hl.rows();
            Hl.conservativeResize(startIndex+3, dimP);
            Hl.block(startIndex,0,3,dimP) = Eigen::MatrixXd::Zero(3,dimP);
            Hl.block(startIndex,6,3,3) = Eigen::Matrix3d::Identity(); // -I
            Hl.block(startIndex,3*it_estimated->second-6,3,3) = Eigen::Matrix3d::Zero(); // zero

            // Fill out N (right invariant)
            startIndex = Nr.rows();
            Nr.conservativeResize(startIndex+3, startIndex+3);
            Nr.block(startIndex,0,3,startIndex) = Eigen::MatrixXd::Zero(3,startIndex);
            Nr.block(0,startIndex,startIndex,3) = Eigen::MatrixXd::Zero(startIndex,3);
            Nr.block(startIndex,startIndex,3,3) = R * it->covariance * R.transpose();

            // Fill out N (left invariant)
            startIndex = Nl.rows();
            Nl.conservativeResize(startIndex+3, startIndex+3);
            Nl.block(startIndex,0,3,startIndex) = Eigen::MatrixXd::Zero(3,startIndex);
            Nl.block(0,startIndex,startIndex,3) = Eigen::MatrixXd::Zero(startIndex,3);
            Nl.block(startIndex,startIndex,3,3) = cov;
    ...
    // Correct state using stacked observation
    Observation obsR(Yr,br,Hr,Nr,PI);
    if (!obsR.empty()) {
        this->CorrectRI(obsR);
    }

    // Correct state using stacked observation
    Observation obsL(Yl,bl,Hl,Nl,PI);
    if (!obsL.empty()) {
        this->CorrectLI(obsL);
    }
