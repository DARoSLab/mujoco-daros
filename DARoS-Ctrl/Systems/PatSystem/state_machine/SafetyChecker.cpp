...
bool SafetyChecker<T>::checkSafeOrientation() {
  if (abs(data->_stateEstimator->getResult().rpy(0)) >= 1.5 ||
      abs(data->_stateEstimator->getResult().rpy(1)) >= 1.5) {
...
bool SafetyChecker<T>::checkPDesFoot() {
  ...
  T maxPDes = data->_pat->_maxLegLength * sin(maxAngle);
  ...
  for (size_t leg = 0; leg < pat_biped::num_legs; leg++) {
    ...
    if (data->_legController->commands[leg].pDes(2) >
        -data->_pat->_maxLegLength / 4) {
      std::cout << "[CONTROL FSM] Safety: PDes leg: " << leg
                << " | coordinate: " << 2 << "\n";
      std::cout << "   commanded: "
                << data->_legController->commands[leg].pDes(2)
                << " | modified: " << -data->_pat->_maxLegLength / 4
                << std::endl;
      data->_legController->commands[leg].pDes(2) =
          -data->_pat->_maxLegLength / 4;
      safePDesFoot = false;
    ...
    if (data->_legController->commands[leg].pDes(2) <
        -data->_pat->_maxLegLength) {
      std::cout << "[CONTROL FSM] Safety: PDes leg: " << leg
                << " | coordinate: " << 2 << "\n";
      std::cout << "   commanded: "
                << data->_legController->commands[leg].pDes(2)
                << " | modified: " << -data->_pat->_maxLegLength
                << std::endl;
      data->_legController->commands[leg].pDes(2) =
          -data->_pat->_maxLegLength;
      safePDesFoot = false;
