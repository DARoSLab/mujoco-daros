...
template<typename T>
LocomotionCtrlRMP<T>::LocomotionCtrlRMP(const ControlFSMData<T> * data):WBC_Ctrl<T>(data)
{
...
template<typename T>
LocomotionCtrlRMP<T>::~LocomotionCtrlRMP(){
...
template<typename T>
void LocomotionCtrlRMP<T>::_ContactTaskUpdate(void* input){
  //this->_EstimateContactForces(); // sehwan - this prints out gabriel's estimated contact forces

  _input_data = static_cast<LocomotionCtrlRMPData<T>* >(input);
...
template<typename T>
void LocomotionCtrlRMP<T>::_ParameterSetup(const PatParameters* param){
...
template<typename T>
void LocomotionCtrlRMP<T>::_CleanUp(){
  this->_contact_list.clear();
  this->_task_list.clear();
}

template<typename T>
void LocomotionCtrlRMP<T>::_LCM_PublishData() {
  int iter(0);
  for(size_t leg(0); leg<pat_biped::num_legs; ++leg){
...
template class LocomotionCtrlRMP<float>;
//template class LocomotionCtrlRMP<double>;
#endif
