...
template <typename T>
BalanceStandState<T>::BalanceStandState(ObserverManager<T>* obs_manager, PrestoeBipedSystem<T>* prestoe_system):
  ...
  _ReadConfig(THIS_COM"/Systems/PrestoeBipedSystem/Configs/standing_state.yaml");
  _jtorque_pos_cmd = new JTorquePosCommand<T>(prestoe::num_act_joint);

  _wbc_ctrl = new PrestoeStandCtrl<T>(&_fb_model, THIS_COM"/Systems/PrestoeBipedSystem/Configs/standing_state.yaml");
